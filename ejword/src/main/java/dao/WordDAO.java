...
public class WordDAO {
	...
	public List<Word> getListBySearchWord(String searchWord, String mode, int limit, int offset){
		...
		searchWord = modifiSearchWord(searchWord, mode);
		try {
			this.connect();
			ps = db.prepareStatement("SELECT * FROM words WHERE title LIKE ? LIMIT ? OFFSET ?");//LIMIT ?で最大検索件数
			ps.setString(1, searchWord);
			ps.setInt(2, limit);
			ps.setInt(3, offset);			
			//System.out.println("デバッグ"+ps);
			rs = ps.executeQuery();
	...
	private String modifiSearchWord(String searchWord, String mode) {
